...
// Compile: nvcc sn-vm-gpu-1b.cu -o sn-vm-gpu-1b -gencode arch=compute_35,code=compute_35 -Wno-deprecated-gpu-targets
...
__global__ void sn_vm_gpu(int *b, int *d, int *r, int *mu, int m, int n, int *y, int maxk, int dbg)
{
    int j = threadIdx.x;
...
print_vect(mu,m);
  ...
  t1=seconds();
  sn_vm_gpu<<<grid, block>>>(d_b, d_d, d_r, d_mu, m, n, d_y, maxk, dbg);
